    ...
    for child in src_node:
        ...
        if tag == "tavatekst":
            ...
            for sub in child:
                ...
                if sub_tag in {"b", "i", "sup", "sub"}:
                    el = etree.SubElement(parent_akn, _q(sub_tag))
                    el.text = sub.text or ""
                    if sub.tail:
                        _append_text(parent_akn, _clean_tail(sub.tail))
            if child.tail:
...
def _convert_peatykk(
    ...
    h = p_src.findtext("{*}peatykkPealkiri")
    if h and h.strip():
...
def riigi_teataja_to_akn(
    ...
    *,
    ...
    etree.SubElement(
        f_man, _q("FRBRdate"), date=datetime.now().strftime("%Y-%m-%d"), name="generation"
    )
