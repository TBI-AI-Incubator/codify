...
1. **A body registry** — `data/jurisdictions/supranational_bodies.json`. Each entry
   is `{code, aliases?, ...}` (`aliases` optional; codes include e.g. `un`, `eu`,
   `ohada`, `african-union`, `caricom`, `asean`, `wto` — the file is the full list).
...
Steps 3–4 are the only ones that need new _content_ (a lens config + the body's
corpus); steps 1–2 are pure config edits. The abstraction that makes this work is
