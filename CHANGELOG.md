...
13. `upsert_embedding` takes `version_id` and `jurisdiction_id` as required
    keywords; `get_or_create_jurisdiction` creates the jurisdiction's partition,
    and an embedding for a jurisdiction written around it fails at insert.
